...
class InIConfig():
	...
	def convertPathOS(self):
		tmpstr = ""
		for keys in self.iniDict:
			for params in self.iniDict[str(keys)]:
				if "/" in self.iniDict[str(keys)][str(params)] or "\\" in self.iniDict[str(keys)][str(params)]:
					tmpstr = str(pathlib.Path(self.iniDict[str(keys)][str(params)]))
					self.iniDict[str(keys)][str(params)] = tmpstr
		del tmpstr
